...
struct ScoreFinalizationView: View {
    ...
    var body: some View {
        ...
        if(!exit){
            NavigationView {
                VStack{
                    CircularProgressView(fullscore: CGFloat(dataManager.lists[spellingList.number].words.count), score: CGFloat(dataManager.lists[spellingList.number].pastResult!.score))
                        .padding()
                        .frame(width: UIScreen.main.bounds.size.width * 0.7)
                    if edit == false {
                        Text(encouragementMessage(score:Int(dataManager.lists[spellingList.number].pastResult!.score/dataManager.lists[spellingList.number].words.count*100)))
                            .padding()
                    }
                    if image != nil {
                        Button{
...
struct ScoreFinalizationView_Previews: PreviewProvider {
    static var previews: some View {
        ScoreFinalizationView(spellingList: SpellingList(words: [SpellingWord(word: "你好")], name: "HALLO"))
    }
}
